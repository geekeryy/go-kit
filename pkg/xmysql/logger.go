...
func NewLogger(writer Writer, config logger.Config) logger.Interface {
	var (
		infoStr      = "%s\n[info] "
		warnStr      = "%s\n[warn] "
		errStr       = "%s\n[error] "
...
// LogMode log mode
func (l *_logger) LogMode(level logger.LogLevel) logger.Interface {
	newlogger := *l
...
func (l _logger) Trace(ctx context.Context, begin time.Time, fc func() (string, int64), err error) {
